...
This alias inherits the default `vc-implement` living-tree contract: run in the operator's current checkout and current branch. Do not create, switch to, or move execution into a git worktree unless the operator explicitly asks for a worktree in this prompt.
...
```bash
vibecrafted implement <agent>     # default
vibecrafted justdo <agent>        # alias, identical behavior
vc-implement <agent>              # shell helper, default
...
default brand (`codex-skill-implement`, `claude-skill-implement`,
